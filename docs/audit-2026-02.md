...
| # | Élément | Statut |
|---|---------|--------|
| 1 | Colonnes OCR dans `eleves` (`ocr_provider`, `ocr_model`, `ocr_cost`, `ocr_duration_ms`) | **Supprimées** du schema |
| 2 | `app/main.py` — fichier vide/stub | N'existait déjà plus |
| 3 | `src/api/main.py` + `src/api/config.py` — app FastAPI standalone | **Supprimés** (jamais utilisés, `run.py` monte les routers directement) |
| 4 | `pdfplumber_parser.py` — parser legacy complet | Conservé (accessible via `PDF_PARSER_TYPE=pdfplumber`, rétrocompatibilité légitime) |
...
| 2 | Batch generation via API ne chargeait pas les few-shot (contrairement au path UI) | `src/api/routers/syntheses.py` | MOYENNE | **Corrigé** — les deux chemins passent maintenant `use_fewshot=True` |
